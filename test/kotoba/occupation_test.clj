...
(deftest human-gap-referral-draft-routing
  (testing ":on-site-recurring gap-shape routes to the temp-staffing agency (isic-7820)"
    (let [draft (occupation/human-gap-referral-draft
                 "1321" {:task "cover a night shift on the floor"
                         :reason :missing-technology
                         :duration :recurring
                         :location :on-site
                         :urgency :normal})]
      (is (= "cloud-itonami-isic-7820" (:target-actor draft)))
      (is (= "on-site-recurring -> temporary staffing agency is employer of record"
             (:routing-reason draft)))))
  (testing ":permanent-role gap-shape routes to the permanent-placement agency (isic-7810), regardless of location"
    (let [draft (occupation/human-gap-referral-draft
                 "7126" {:task "hire a permanent second pipefitter"
                         :reason :no-automation-path
                         :duration :permanent
                         :location :on-site
                         :urgency :low})]
      (is (= "cloud-itonami-isic-7810" (:target-actor draft)))
      (is (= "permanent-role -> permanent placement agency" (:routing-reason draft)))))
  (testing ":one-off-remote-or-cognitive gap-shape routes to the BPO/task-matching actor (isic-8299)"
    (let [draft (occupation/human-gap-referral-draft
                 "8332" {:task "one-off remote route-planning review"
                         :reason :other
                         :duration :one-off
                         :location :remote
                         :urgency :normal})]
      (is (= "cloud-itonami-isic-8299" (:target-actor draft)))
      (is (= "one-off-remote-or-cognitive -> BPO/task-matching, contracted independent operators"
             (:routing-reason draft)))))
  (testing ":public-recruit-reach gap-shape is the fallback for unrecognized/missing :duration+:location"
    (let [draft (occupation/human-gap-referral-draft
                 "1321" {:task "widen candidate reach for a hard-to-fill role"
                         :reason :other
                         :duration :unknown
                         :location :unknown
                         :urgency :normal})]
      (is (= "cloud-itonami-isic-6399" (:target-actor draft)))
      (is (= "public-recruit-reach -> public meta job board" (:routing-reason draft))))))

(deftest human-gap-referral-draft-shape
  (testing "business-id and occupation-context are populated from the real ISCO registry entry"
    (doseq [isco ["1321" "7126" "8332"]]
      (let [draft (occupation/human-gap-referral-draft
                   isco {:task "some task the robot can't do"
                         :reason :missing-technology
                         :duration :one-off
                         :location :remote
                         :urgency :normal})]
        (is (= (:business-id (occupation/get-occupation isco)) (:business-id draft)))
        (is (some? (:business-id draft)))
        (is (= (occupation/execution-plan isco) (:occupation-context draft))))))
  (testing "the output map's keys are exactly the documented set -- no stray PII-shaped keys"
    (let [draft (occupation/human-gap-referral-draft
                 "1321" {:task "cover a shift"
                         :reason :missing-technology
                         :duration :recurring
                         :location :on-site
                         :urgency :normal})]
      ...
      (is (not-any? #{:name :email :phone :address :person :worker :contact}
                     (keys draft)))))
  (testing "two calls produce different :draft-id values -- each call is a distinct
            real-world gap-detection occurrence, even for identical isco+gap input"
    (let [gap {:task "cover a shift" :reason :missing-technology
               :duration :recurring :location :on-site :urgency :normal}
          draft-1 (occupation/human-gap-referral-draft "1321" gap)
          draft-2 (occupation/human-gap-referral-draft "1321" gap)]
      (is (not= (:draft-id draft-1) (:draft-id draft-2)))))
  (testing ":drafted-at-state is always the fixed ledger-state term :referral-drafted
            (precedent ADR-2607131000: \"6399's ledger shows referral-drafted\"), never
            the occupation's own :maturity tier"
    (let [draft (occupation/human-gap-referral-draft
                 "8332" {:task "t" :reason :other :duration :one-off
                         :location :remote :urgency :low})]
...
(deftest signwriters-decorative-painters-engravers-etchers-7316-implemented
  (testing "7316 (Signwriters, Decorative Painters, Engravers and
            Etchers) promoted to :implemented -- SignCraftActor (Sign
            & Decorative Craft Advisor ⊣ SignCraftGovernor); closed
            four-op proposal allowlist (:log-work-record,
            :schedule-crew-operation, :flag-safety-concern,
            :coordinate-supply-order) -- a sign & decorative-craft
            workshop scheduling/logistics coordination robot ONLY,
            never direct painting/engraving-execution authority and
            never chemical-safety-clearance authority. Signwriters,
            Decorative Painters, Engravers and Etchers work with
            paints, solvents and etching chemicals (VOC/chemical-fume
            exposure hazard) alongside precision hand-craft work
            (engraving/etching tools) -- two hazard dimensions at
            once -- so this actor has ZERO authority to directly
            finalize a painting/engraving-execution decision (e.g.
            deciding to proceed with a specific signwriting,
            decorative-painting, engraving or etching procedure), or a
            workshop-chemical-safety-clearance decision, or override a
            shop safety officer's judgment: no such op exists anywhere
            in the closed allowlist (structurally absent, not merely
            gated), confirmed by the governor's closed op-allowlist
            HARD check (:unknown-op), a second independent HARD check
            naming nine concretely-forbidden ops
            (:finalize-painting-execution-decision,
            :finalize-engraving-execution-decision,
            :authorize-painting-operation,
            :authorize-engraving-operation,
            :proceed-with-painting-execution-procedure,
            :proceed-with-engraving-execution-procedure,
            :declare-workshop-chemical-safety-cleared,
            :override-safety-officer-judgment,
            :override-shop-safety-officer-judgment), and a
            content-based scope-exclusion HARD check
            (:scope-excluded-action) phrased as finalization/execution
            ACTIONS (never bare nouns, e.g. \"finalize the painting
            operation\", \"declare the workshop chemical-safety
            ...
            proposals for all four ops never self-trip the
            scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains the bare nouns \"paint\",
            \"solvent\", \"engraving\", \"etching\" and \"shop safety
            officer\" (e.g. a :schedule-crew-operation rationale
            naming a \"signwriting/decorative-painting task\", a
            :flag-safety-concern concern routed for shop safety
            officer review). :flag-safety-concern always escalates and
            is never auto-commit-eligible; a :coordinate-supply-order
            above the registered cost threshold (2000, inclusive
            ...
            boundary) escalates -- not a hard block, routine
            paint/solvent/etching-materials procurement above the
            registered threshold, not itself unsafe unlike a
            painting/engraving-execution, chemical-safety-clearance,
            or safety-officer-override attempt. 26 tests / 56
            assertions green (cloud-itonami-isco-7316, ADR-2799007316,
            independently re-verified against a fresh clone of the
            pushed repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this edit (145 spec / 291
            ...
            a prior comment's delta."
    (is (= :implemented (occupation/maturity "7316")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-7316"
...
(deftest shoemakers-and-related-workers-7536-implemented
  (testing "7536 (Shoemakers and Related Workers) promoted to
            :implemented -- ShoeCoordActor (Shoemaking Workshop
            Coordination Advisor ⊣ ShoeCoordGovernor); closed
            proposal-op allowlist (:log-work-record,
            :schedule-crew-operation, :flag-safety-concern,
            :coordinate-supply-order) -- a shoemaking workshop
            scheduling/logistics-coordination robot ONLY. Shoemakers
            use cutting tools (knives, cutting dies) and handle
            adhesives/glues (solvent-fume exposure) -- standard
            workshop hazard combining cutting-tool injury and
            chemical-fume exposure, so this actor has ZERO authority
            to directly finalize a shoemaking-execution decision (e.g.
            deciding to proceed with a specific cutting operation) or
            a workshop-safety-clearance decision (e.g. declaring the
            workshop safety cleared for operation), or override a shop
            safety officer's judgment: no such op exists anywhere in
            the closed allowlist (structurally absent, not merely
            gated), confirmed by the governor's closed op-allowlist
            HARD check (:unknown-op), a second independent HARD check
            naming ten concretely-forbidden ops
            (:finalize-shoemaking-decision,
            :finalize-shoemaking-operation, :finalize-cutting-operation,
            ...
            scope-exclusion HARD check (:scope-excluded-action) phrased
            as finalization/execution ACTIONS (never bare nouns, e.g.
            \"finalize the shoemaking operation\", \"declare the
            workshop safety cleared\", \"override the shop safety
            officer's judgment\"), and independently-verified maker/
            workshop provenance HARD checks (:no-maker, :no-workshop)
            -- verified via a dedicated regression test that the
            default mock advisor's proposals for all four ops never
            self-trip the scope-exclusion guard, even though this
            actor's own vocabulary legitimately contains the bare
            nouns \"shoe\", \"leather\", \"sole\", \"adhesive\",
            \"cutting\" and \"shop safety officer\" (e.g. a
            :schedule-crew-operation rationale naming a \"shoemaking
            task\", a :flag-safety-concern concern routed for shop
            safety officer review). :flag-safety-concern always
            escalates and is never auto-commit-eligible; a
            :coordinate-supply-order above the registered cost
            threshold (2000, inclusive boundary verified by
            ok-supply-order-at-threshold-boundary) escalates -- not a
            hard block, routine leather/sole/adhesive-materials
            procurement above the registered threshold, not itself
            unsafe unlike a shoemaking-execution,
            workshop-safety-clearance, or safety-officer-override
            attempt. 31 tests / 67 assertions green
            (cloud-itonami-isco-7536, ADR-2799007536, independently
            re-verified against a fresh clone of the pushed
            repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this entry's own promotion
            (137 spec / 299 implemented / 0 blueprint at that fetch,
            before this entry's own promotion), reflecting cumulative
            concurrent sibling landings in this same batch, not
            hand-derived from a prior comment's delta."
    (is (= :implemented (occupation/maturity "7536")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-7536"
...
(deftest cement-stone-mineral-products-machine-operators-8114-implemented
  (testing "8114 (Cement, Stone and Other Mineral Products Machine
            Operators) promoted to :implemented -- MineralPlantCoordActor
            (Mineral Products Plant Coordination Advisor ⊣
            MineralPlantCoordGovernor); closed four-op proposal allowlist
            (:log-work-record, :schedule-crew-operation,
            :flag-safety-concern, :coordinate-supply-order) -- a cement/
            stone/mineral-products plant scheduling/logistics
            coordination robot ONLY, never direct machine-operation-
            execution or plant-safety-clearance authority. Cement, stone
            and other mineral products machine operators run heavy
            industrial forming/pressing/kiln equipment -- heavy-machinery
            hazard (crush/entanglement injury) plus dust/heat exposure --
            so this actor has ZERO authority to directly finalize a
            machine-operation-execution decision (e.g. deciding to run
            or proceed with a specific forming/pressing/kiln machine
            cycle) or a plant-safety-clearance decision (e.g. declaring
            the plant floor safety-cleared), or to override a plant
            safety officer's judgment: no such op exists anywhere in the
            closed allowlist (structurally absent, not merely gated),
            confirmed by the governor's closed op-allowlist HARD check
            (:unknown-op), a content-based scope-exclusion HARD check
            (:scope-excluded-action) phrased as finalization/execution
            ACTIONS (never bare nouns, e.g. \"proceed with the machine
            operation\", \"declare the plant safety cleared\",
            \"override the plant safety officer's judgment\"), and
            independently-verified operator/plant provenance HARD checks
            (:no-operator, :no-plant -- a registered record alone is not
            enough). Verified via a dedicated regression test that the
            default mock advisor's proposals for all four ops never
            self-trip the scope-exclusion guard, even though this
            actor's own vocabulary legitimately contains bare nouns like
            \"cement\", \"kiln\" and \"dust\" (e.g. a routine
            :log-work-record/:schedule-crew-operation rationale naming a
            \"production-run task\", a :flag-safety-concern concern
            naming \"dust-exposure-risk\" and routed for plant safety
            officer review). :flag-safety-concern ALWAYS escalates and is
            never auto-commit-eligible, no confidence-level exception,
            ever; a :coordinate-supply-order above the registered cost
            threshold (2000, inclusive boundary verified by
            ok-supply-order-at-threshold-boundary) escalates -- not a
            hard block, routine raw-material/spare-parts procurement
            above the registered threshold, not itself unsafe unlike a
            machine-operation-execution or plant-safety-clearance
            finalization attempt. 21 tests / 45 assertions green
            (cloud-itonami-isco-8114, ADR-2799008114, independently
            re-verified against a fresh clone of the pushed repository).
            Counts re-verified live via (occupation/maturity-summary)
            against a freshly re-fetched origin/main immediately after
            this promotion, reflecting cumulative concurrent sibling
            landings in this same batch, not hand-derived from a prior
    ...
    (is (= :implemented (occupation/maturity "8114")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8114"
           (:repo (occupation/get-occupation "8114"))))
    (is (= "cloud-itonami-isco-8114"
           (:business-id (occupation/get-occupation "8114"))))))

(deftest metal-finishing-plating-coating-machine-operators-8122-implemented
  (testing "8122 (Metal Finishing, Plating and Coating Machine Operators)
            promoted to :implemented -- PlatingCoordActor (Plating Plant
            Scheduling Coordination Advisor ⊣ PlatingCoordGovernor);
            closed four-op proposal allowlist (:log-work-record,
            :schedule-crew-operation, :flag-safety-concern,
            :coordinate-supply-order) -- a plant scheduling/logistics
            coordination robot ONLY, never plating-equipment operation
            or chemical-safety-clearance authority. Metal Finishing,
            Plating and Coating Machine Operators run electroplating
            lines using chemical baths (historically including
            cyanide-based and chromium-based solutions) plus electrical
            hazard from electroplating current -- a real
            chemical-exposure and electrical hazard domain -- so this
            actor has ZERO authority to finalize a
            plating-operation-execution decision, finalize a
            chemical-safety-clearance decision, or override a plant
            safety officer's judgment: no such op exists anywhere in
            the closed allowlist (structurally absent, not merely
            gated), confirmed by the governor's closed op-allowlist
            ...
            finalization/execution ACTION PHRASES (never bare nouns,
            e.g. \"finalize the plating operation\", \"declare the
            chemical bath safety cleared\", \"override the plant
            safety officer's judgment\"), and independently-verified
            plater/facility provenance HARD checks (:no-plater,
            :no-facility -- a registered record alone is not enough).
            Verified via a dedicated regression test that the default
            mock advisor's proposals for all four ops never self-trip
            the scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like
            \"cyanide\", \"chromium\" and \"electroplating\" (e.g. a
            self-trip regression task literally reading \"routine
            electroplating task with cyanide and chromium plating
            bath\"). This actor never operates plating equipment
            itself: :log-work-record covers production-run/inventory/
            progress data logging only, :schedule-crew-operation covers
            crew/shift/task scheduling only, :coordinate-supply-order
            covers plating-chemicals/raw-material procurement only, and
            :flag-safety-concern ALWAYS escalates and is never
            auto-commit-eligible, no confidence-level exception, ever;
            a :coordinate-supply-order above the registered cost
            ...
            (cloud-itonami-isco-8122, ADR-2799008122, independently
            re-verified against a fresh clone of the pushed
            repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this promotion (126 spec /
            310 implemented / 0 blueprint at that fetch), reflecting
            cumulative concurrent sibling landings in this same batch,
            not hand-derived from a prior comment's delta -- this
            deftest deliberately does NOT touch the maturity-tier
            aggregate count assertion above, which only asserts
            :total 436 (invariant across maturity promotions), not a
            hand-derived spec/implemented split."
    (is (= :implemented (occupation/maturity "8122")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8122"
...
(deftest paper-products-machine-operators-8143-implemented
  (testing "8143 (Paper Products Machine Operators) promoted to
            :implemented -- PaperCoordActor (Paper Products Plant
            Scheduling Coordination Advisor ⊣ PaperCoordGovernor);
            closed four-op proposal allowlist (:log-work-record,
            :schedule-crew-operation, :flag-safety-concern,
            :coordinate-supply-order) -- a plant scheduling/logistics
            coordination robot ONLY, never converting-equipment
            operation or plant-safety-clearance authority. Paper
            Products Machine Operators run converting/cutting/folding
            equipment (box-making, bag-making, tissue-converting
            machines) -- a real heavy-machinery hazard domain (crush,
            entanglement and cutting-blade injury from rollers and
            blades) -- so this actor has ZERO authority to finalize a
            ...
            HARD check (:scope-excluded-action) phrased as
            finalization/execution ACTION PHRASES (never bare nouns,
            e.g. \"finalize the converting operation\", \"declare the
            plant safety cleared\", \"override the plant safety
            officer's judgment\"), and independently-verified
            converter/facility provenance HARD checks (:no-converter,
            :no-facility -- a registered record alone is not enough).
            Verified via a dedicated regression test that the default
            mock advisor's proposals for all four ops never self-trip
            the scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like \"paper\",
            \"blade\" and \"roller\" (e.g. a self-trip regression task
            literally reading \"routine converting task with paper
            rollers and cutting blades\"). This actor never operates
            converting equipment itself: :log-work-record covers
            production-run/inventory/progress data logging only,
            :schedule-crew-operation covers crew/shift/task scheduling
            only, :coordinate-supply-order covers raw-paper/converting-
            materials procurement only, and :flag-safety-concern
            ALWAYS escalates and is never auto-commit-eligible, no
            ...
            escalates) escalates -- not a hard block. 30 tests / 65
            assertions green (cloud-itonami-isco-8143, ADR-2799008143,
            independently re-verified against a fresh clone of the
            pushed repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this promotion (122 spec /
            314 implemented / 0 blueprint at that fetch), reflecting
            cumulative concurrent sibling landings in this same batch,
            not hand-derived from a prior comment's delta -- this
            deftest deliberately does NOT touch the maturity-tier
            aggregate count assertion above, which only asserts
    ...
    (is (= :implemented (occupation/maturity "8143")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8143"
...
(deftest laundry-machine-operators-8157-implemented
  (testing "8157 (Laundry Machine Operators) promoted to :implemented --
            LaundryCoordActor (Laundry Plant Scheduling Coordination
            Advisor ⊣ LaundryCoordGovernor); closed four-op proposal
            allowlist (:log-work-record, :schedule-crew-operation,
            :flag-safety-concern, :coordinate-supply-order) -- a plant
            scheduling/logistics coordination robot ONLY, never
            laundry-machine operation or plant-safety-clearance
            authority. Laundry Machine Operators run industrial
            washers, dryers and pressing equipment -- a real
            heat/steam-burn hazard, an entanglement hazard from
            rotating drums and pressing mechanisms, and
            detergent/chemical exposure -- so this actor has ZERO
            authority to finalize a machine-operation-execution
            ...
            (:scope-excluded-action) phrased as finalization/execution
            ACTION PHRASES (never bare nouns, e.g. \"finalize the
            laundry-machine operation\", \"declare the plant safety
            cleared\", \"override the plant safety officer's
            judgment\"), and independently-verified launderer/plant
            provenance HARD checks (:no-launderer, :no-plant -- a
            registered record alone is not enough). Verified via a
            dedicated regression test that the default mock advisor's
            proposals for all four ops never self-trip the
            scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like
            \"detergent\", \"steam\" and \"drum\" (e.g. a self-trip
            regression task literally reading \"routine washer drum,
            dryer heat and press steam maintenance task with
            detergent/linen-stock check\"). This actor never operates
            laundry-machine equipment itself: :log-work-record covers
            production-run/inventory/progress data logging only,
            :schedule-crew-operation covers crew/shift/task scheduling
            only, :coordinate-supply-order covers detergent/linen-stock
            ...
            escalates -- not a hard block. 37 tests / 79 assertions
            green (cloud-itonami-isco-8157, ADR-2799008157,
            independently re-verified against a fresh clone of the
            pushed repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this promotion (119 spec /
            317 implemented / 0 blueprint before this entry's own
            edit, 117 spec / 319 implemented / 0 blueprint at a fresh
            re-fetch immediately after), reflecting cumulative
            concurrent sibling landings in this same batch, not
            hand-derived from a prior comment's delta -- this deftest
            deliberately does NOT touch the maturity-tier aggregate
            ...
            left exactly as found rather than fought over, per the
            race-safe convention for this hot-contention batch."
    (is (= :implemented (occupation/maturity "8157")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8157"
           (:repo (occupation/get-occupation "8157"))))
    (is (= "cloud-itonami-isco-8157"
           (:business-id (occupation/get-occupation "8157"))))))

(deftest shoemaking-and-related-machine-operators-8156-implemented
  (testing "8156 (Shoemaking and Related Machine Operators) promoted to
            :implemented -- ShoeCoordActor (Shoemaking and Related
            Machine Operators Plant Scheduling Coordination Advisor ⊣
            ShoeCoordGovernor); closed four-op proposal allowlist
            (:log-work-record, :schedule-crew-operation,
            :flag-safety-concern, :coordinate-supply-order) -- a plant
            scheduling/logistics coordination robot ONLY, never
            shoemaking-machinery operation or plant-safety-clearance
            authority. Shoemaking and Related Machine Operators run
            cutting/stitching/molding machinery for footwear
            manufacture (cutting-die presses, stitching machines,
            sole-molding/lasting machines) -- a real heavy-machinery
            hazard domain (crush and cutting-blade injury from presses
            and cutting dies) plus adhesive-fume exposure -- so this
            actor has ZERO authority to finalize a
            machine-operation-execution decision, finalize a
            plant-safety-clearance decision, or override a plant
            safety officer's judgment: no such op exists anywhere in
            ...
            HARD check (:scope-excluded-action) phrased as
            finalization/execution ACTION PHRASES (never bare nouns,
            e.g. \"finalize the shoemaking-machine operation\",
            \"declare the plant safety cleared\", \"override the plant
            safety officer's judgment\"), and independently-verified
            operator/facility provenance HARD checks (:no-operator,
            :no-facility -- a registered record alone is not enough).
            Verified via a dedicated regression test that the default
            mock advisor's proposals for all four ops never self-trip
            the scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like \"shoe\",
            \"leather\" and \"cutting die\" (e.g. a self-trip
            regression task literally reading \"routine shoemaking
            task with leather uppers and cutting dies\"). This actor
            never operates shoemaking machinery itself:
            :log-work-record covers production-run/inventory/progress
            data logging only, :schedule-crew-operation covers
            crew/shift/task scheduling only, :coordinate-supply-order
            covers leather/sole/adhesive-materials procurement only,
            ...
            ok-supply-order-at-threshold-boundary -- exactly-at-
            threshold commits, over-threshold escalates) escalates --
            not a hard block. 30 tests / 65 assertions green
            (cloud-itonami-isco-8156, ADR-2799008156, independently
            re-verified against a fresh clone of the pushed
            repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this test-file edit (117
            spec / 319 implemented / 0 blueprint at that fetch,
            ...
            left exactly as found rather than fought over, per the
            race-safe convention for this hot-contention batch."
    (is (= :implemented (occupation/maturity "8156")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8156"
...
(deftest fur-and-leather-preparing-machine-operators-8155-implemented
  (testing "8155 (Fur and Leather Preparing Machine Operators) promoted
            to :implemented -- FurLeatherCoordActor (Fur and Leather
            Preparing Plant Scheduling Coordination Advisor ⊣
            FurLeatherCoordGovernor); closed four-op proposal
            allowlist (:log-work-record, :schedule-crew-operation,
            :flag-safety-concern, :coordinate-supply-order) -- a plant
            scheduling/logistics coordination robot ONLY, never
            fur/leather preparing machinery operation or
            chemical-safety-clearance authority. Fur and Leather
            Preparing Machine Operators run tanning/dressing machinery
            (fleshing, tanning-drum, dyeing, splitting and staking
            machines) using chemical processing agents -- a real
            chemical-exposure hazard domain, plus heavy-machinery
            hazard (crush/entanglement injury from rollers and drums)
            -- so this actor has ZERO authority to finalize a
            processing-operation-execution decision, finalize a
            chemical-safety-clearance decision, or override a plant
            safety officer's judgment: no such op exists anywhere in
            the closed allowlist (structurally absent, not merely
            gated), confirmed by the governor's closed op-allowlist
            ...
            HARD check (:scope-excluded-action) phrased as
            finalization/execution ACTION PHRASES (never bare nouns,
            e.g. \"finalize the tanning operation\", \"declare the
            plant safety cleared\", \"override the plant safety
            officer's judgment\"), and independently-verified
            operator/plant provenance HARD checks (:no-operator,
            :no-plant -- a registered record alone is not enough).
            ...
            mock advisor's proposals for all four ops never self-trip
            the scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like \"pelt\",
            \"fur\" and \"chemical\" (e.g. a self-trip regression task
            literally reading \"routine fur and leather preparing task
            with tanning drum rollers and chemical processing
            agents\"). This actor never operates fur/leather preparing
            machinery itself: :log-work-record covers
            production-run/inventory/progress data logging only,
            :schedule-crew-operation covers crew/shift/task scheduling
            only, :coordinate-supply-order covers
            processing-chemicals/raw-hide procurement only, and
            :flag-safety-concern ALWAYS escalates and is never
            auto-commit-eligible, no confidence-level exception, ever;
            a :coordinate-supply-order above the registered cost
            threshold (2000, boundary verified exclusive by
            ok-supply-order-at-threshold-boundary -- exactly-at-
            threshold commits, over-threshold escalates) escalates --
            not a hard block. 34 tests / 73 assertions green
            (cloud-itonami-isco-8155, ADR-2799008155, independently
            re-verified against a fresh clone of the pushed
            repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this test-file edit (116
            spec / 320 implemented / 0 blueprint at that fetch,
            already reflecting this entry's own promotion and
            cumulative concurrent sibling landings in this same batch
            -- not hand-derived from a prior comment's delta -- this
            deftest deliberately does NOT touch the maturity-tier
            ...
            left exactly as found rather than fought over, per the
            race-safe convention for this hot-contention batch."
    (is (= :implemented (occupation/maturity "8155")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8155"
...
(deftest glass-and-ceramics-plant-operators-8181-implemented
  (testing "8181 (Glass and Ceramics Plant Operators) promoted to
            :implemented -- GlassCeramicsPlantCoordActor (Glass and
            Ceramics Plant Coordination Advisor ⊣
            GlassCeramicsPlantCoordGovernor); closed four-op proposal
            allowlist (:log-work-record, :schedule-crew-operation,
            :flag-safety-concern, :coordinate-supply-order) -- a
            glass/ceramics plant scheduling/logistics coordination
            robot ONLY, never glass-melting furnace or ceramics kiln
            operation itself or direct
            plant-operation-execution/plant-safety-clearance
            authority. ISCO-08 8181 runs high-temperature
            glass-melting furnaces and ceramics kilns -- severe
            burn-hazard risk plus heavy-machinery hazard from
            forming/handling equipment -- so this actor has ZERO
            authority to directly finalize a plant-operation-execution
            decision, finalize a plant-safety-clearance decision, or
            override a plant safety officer's judgment: no such op
            exists anywhere in the closed allowlist (structurally
            absent, not merely gated), confirmed by the governor's
            ...
            (:scope-excluded-action) phrased as finalization/execution
            ACTION PHRASES (never bare nouns, e.g. \"finalize the
            furnace-operation decision\", \"declare the plant safety
            cleared\", \"override the plant safety officer's
            judgment\"), and independently-verified operator/plant
            provenance HARD checks (:no-operator, :no-plant -- a
            registered record alone is not enough). Verified via a
            ...
            proposals for all four ops never self-trip the
            scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like \"glass\",
            \"ceramics\", \"kiln\" and \"furnace\" (e.g. a
            :schedule-crew-operation rationale naming a routine
            production-run task, a :flag-safety-concern rationale
            naming a burn-hazard concern routed for plant safety
            officer review). This actor never operates furnace/kiln
            equipment itself: :log-work-record covers production-run/
            inventory/progress data logging only,
            :schedule-crew-operation covers crew/shift/task scheduling
            only, :coordinate-supply-order covers raw-material/
            refractory-materials procurement only, and
            :flag-safety-concern ALWAYS escalates and is never
            auto-commit-eligible, no confidence-level exception, ever;
            a :coordinate-supply-order above the registered cost
            threshold (2000, exclusive boundary verified by
            ok-supply-order-at-threshold-boundary) escalates -- not a
            hard block, routine raw-material procurement above the
            registered threshold is not itself unsafe unlike a
            plant-operation-execution or
            plant-safety-clearance/safety-officer-override attempt.
            The reference repo mirrored for module SHAPE was
            cloud-itonami-isco-8114 (Cement, Stone and Other Mineral
            Products Machine Operators), independently re-verified:
            fresh clone, clojure -M:test green 24 tests / 52
            assertions, 0 failures / 0 errors, before mirroring. 24
            tests / 52 assertions green (cloud-itonami-isco-8181,
            ADR-2799008181, independently re-verified against a fresh
            clone of the pushed repository). Counts re-verified live
            via (occupation/maturity-summary) against a freshly
            re-fetched origin/main immediately before this entry's own
            promotion, already reflecting this entry's own promotion
            and cumulative concurrent sibling landings in this same
            batch of 6 -- not hand-derived from a prior comment's
            ...
            batch."
    (is (= :implemented (occupation/maturity "8181")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8181"
...
(deftest electrical-electronic-equipment-assemblers-8212-implemented
  (testing "8212 (Electrical and Electronic Equipment Assemblers)
            promoted to :implemented -- ElecAssemblyCoordActor
            (Electrical/Electronic Assembly Line Scheduling Coordination
            Advisor ⊣ ElecAssemblyCoordGovernor); closed four-op
            proposal allowlist (:log-work-record,
            :schedule-crew-operation, :flag-safety-concern,
            :coordinate-supply-order) -- a line scheduling/logistics
            coordination robot ONLY, never assembly-execution or
            line-safety-clearance authority. Electrical and Electronic
            Equipment Assemblers assemble circuit boards and electrical
            equipment on production lines, presenting both an
            electrical-shock hazard and an ESD (electrostatic
            discharge)-sensitive component-handling hazard, so this
            ...
            officer's judgment\"), and independently-verified
            worker/line provenance HARD checks (:no-worker,
            :no-line -- a registered record alone is not enough).
            Verified via a dedicated regression test that the default
            mock advisor's proposals for all four ops never self-trip
            the scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like
            \"circuit board\", \"ESD\" and \"voltage\" (e.g. a
            self-trip regression task literally reading \"routine
            circuit board assembly task with ESD-sensitive components
            under line voltage\"). This actor never performs assembly
            work itself: :log-work-record covers production-run/
            inventory/progress data logging only,
            :schedule-crew-operation covers crew/shift/task scheduling
            only, :coordinate-supply-order covers
            electronic-components-stock procurement only, and
            :flag-safety-concern ALWAYS escalates and is never
            auto-commit-eligible, no confidence-level exception, ever;
            a :coordinate-supply-order above the registered cost
            threshold (2000, boundary verified exclusive by
            ...
            threshold commits, over-threshold escalates) escalates --
            not a hard block. 33 tests / 71 assertions green
            (cloud-itonami-isco-8212, ADR-2799008212, independently
            re-verified against a fresh clone of the pushed
            repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this entry's own promotion
            (111 spec / 325 implemented / 0 blueprint at that fetch,
            already reflecting this entry's own promotion and
            cumulative concurrent sibling landings in this same batch
            of 6) -- not hand-derived from a prior comment's delta --
            this deftest deliberately does NOT touch the maturity-tier
            ...
            left exactly as found rather than fought over, per the
            race-safe convention for this hot-contention batch."
    (is (= :implemented (occupation/maturity "8212")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-8212"
...
(deftest livestock-farm-labourers-9212-implemented
  (testing "9212 (Livestock Farm Labourers) promoted to :implemented --
            LivestockFarmActor (Livestock Farm Labourer Advisor ⊣
            LivestockFarmGovernor); closed four-op proposal allowlist
            (:log-work-record, :schedule-crew-operation,
            :flag-safety-concern, :coordinate-supply-order) -- a farm
            scheduling/logistics coordination robot ONLY, never
            livestock-handling-execution or farm-safety-clearance
            authority. Livestock Farm Labourers is an ISCO major group 9
            elementary occupation (lower nominal formal-skill
            requirement than the 7xxx/8xxx trades/plant-operator groups
            already landed), but the real-world safety stakes stack
            three ways: animal-handling hazard (kick/trample/bite
            injury), outdoor-exposure hazard (weather, uneven terrain)
            and standard farm-equipment hazard -- so the governance
            SHAPE (independent Governor, closed allowlist, hard/
            escalate split) is NOT simplified relative to the
            higher-skill-tier actors. This actor has ZERO authority to
            finalize a livestock-handling-execution decision (e.g.
            approving a specific handling or restraint procedure),
            finalize a farm-safety-clearance decision, or override a
            farm safety supervisor's judgment: no such op exists
            anywhere in the closed allowlist (structurally absent, not
            merely gated), confirmed by the governor's closed
            op-allowlist HARD check (:unknown-op), a content-based
            scope-exclusion HARD check (:scope-excluded-action) phrased
            as finalization/execution ACTION PHRASES (never bare nouns,
            e.g. \"approve the livestock handling procedure\",
            \"declare the farm safety cleared\", \"override the farm
            safety supervisor's judgment\"), and independently-verified
            worker/farm provenance HARD checks (:no-worker, :no-farm --
            a registered record alone is not enough). Verified via a
            dedicated regression test that the default mock advisor's
            proposals for all four ops never self-trip the
            scope-exclusion guard, even though this actor's own
            vocabulary legitimately contains bare nouns like
            \"livestock\", \"farm\" and \"farm safety supervisor\" (e.g.
            a self-trip regression task literally reading \"routine
            livestock handling task\" with hazard-type
            :animal-behavior-hazard). This actor never performs farm
            labour or livestock handling itself: :log-work-record
            covers feeding/care-log/progress data logging only,
            :schedule-crew-operation covers crew/task scheduling only,
            :coordinate-supply-order covers feed/farm-supplies
            procurement only, and :flag-safety-concern ALWAYS escalates
            and is never auto-commit-eligible, no confidence-level
            exception, ever; a :coordinate-supply-order above the
            registered cost threshold (2000, boundary verified
            ...
            for module SHAPE was cloud-itonami-isco-7511 (Butchers,
            Fishmongers and Related Food Preparers -- the closest
            animal-material-handling hazard-domain analog), independently
            re-cloned and read in full before mirroring, extended with
            a second, independent outdoor-exposure/farm-equipment
            hazard-scope dimension. 24 tests / 52 assertions green
            (cloud-itonami-isco-9212, ADR-2799009212, independently
            re-verified against a fresh clone of the pushed
            repository). Counts re-verified live via
            (occupation/maturity-summary) against a freshly re-fetched
            origin/main immediately before this entry's own promotion
            ({:total 436, :spec 108, :blueprint 0, :implemented 328} at
            that fetch, already reflecting this entry's own promotion
            and cumulative concurrent sibling landings in this same
            batch of 6) -- not hand-derived from a prior comment's
            delta -- this deftest deliberately does NOT touch the
            maturity-tier aggregate count assertion above, which only
            asserts :total 436 (invariant across maturity promotions),
            not a hand-derived spec/implemented split -- that assertion
            is left exactly as found rather than fought over, per the
            race-safe convention for this hot-contention batch."
    (is (= :implemented (occupation/maturity "9212")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-9212"
...
(deftest fishery-and-aquaculture-labourers-9216-implemented
  (testing "9216 (Fishery and Aquaculture Labourers) promoted to
            :implemented -- FisheryAquacultureActor (Fishery and
            Aquaculture Labourer Advisor ⊣ FisheryAquacultureGovernor);
            closed four-op proposal allowlist (:log-work-record,
            :schedule-crew-operation, :flag-safety-concern,
            :coordinate-supply-order) -- a site scheduling/logistics
            coordination robot ONLY, never fishery/aquaculture-work-
            execution or site-safety-clearance authority. Fishery and
            Aquaculture Labourers is an ISCO major group 9 elementary
            occupation (lower nominal formal-skill requirement than the
            7xxx/8xxx trades/plant-operator groups already landed), but
            the real-world safety stakes stack three ways: drowning-risk
            hazard (water-based work, a category comparable in stakes to
            a diving-hazard domain), aquatic-environment-exposure hazard
            (water quality/current/temperature), and outdoor weather/
            terrain-exposure hazard -- so the governance SHAPE (independent
            Governor, closed allowlist, hard/escalate split) is NOT
            simplified relative to the higher-skill-tier actors. This
            actor has ZERO authority to finalize a fishery/aquaculture-
            work-execution decision (e.g. authorizing a water-based
            operation to proceed), finalize a site-safety-clearance
            decision (e.g. declaring a site cleared for safety), or
            override a site safety supervisor's judgment: no such op
            exists anywhere in the closed allowlist (structurally absent,
            not merely gated), confirmed by the governor's closed
            op-allowlist HARD check (:unknown-op), a content-based
            scope-exclusion HARD check (:scope-excluded-action) phrased as
            finalization/execution ACTION PHRASES (never bare nouns, e.g.
            \"authorize the water-based operation to proceed\", \"declare
            the site safety cleared\", \"override the site safety
            supervisor's judgment\"), and independently-verified worker/
            site provenance HARD checks (:no-worker, :no-site -- a
            registered record alone is not enough). Verified via a
            dedicated regression test that the default mock advisor's
            proposals for all four ops never self-trip the scope-exclusion
            guard, even though this actor's own vocabulary legitimately
            contains bare nouns like \"fish\", \"net\", \"boat\" and \"site
            safety supervisor\" (e.g. a self-trip regression task
            literally reading \"routine fish net and boat maintenance
            task\" with hazard-type :drowning-risk). This actor never
            performs fishery or aquaculture work itself: :log-work-record
            covers harvest/task/progress data logging only,
            :schedule-crew-operation covers crew/task scheduling only,
            :coordinate-supply-order covers feed/equipment procurement
            only, and :flag-safety-concern ALWAYS escalates and is never
            ...
            block. The reference repo mirrored for module SHAPE was
            cloud-itonami-isco-9212 (Livestock Farm Labourers -- the
            closest comparable elementary-occupation outdoor-labour
            hazard-domain analog), independently re-cloned and read in
            full before mirroring, extended with an independent
            drowning-risk/aquatic-environment-exposure hazard-scope
            dimension. 24 tests / 52 assertions green
            (cloud-itonami-isco-9216, ADR-2799009216, independently
            re-verified against a fresh clone of the pushed repository).
            Counts re-verified live via (occupation/maturity-summary)
            against a freshly re-fetched origin/main immediately before
            this entry's own append ({:total 436, :spec 102, :blueprint 0,
            :implemented 334} at that fetch, already reflecting this
            entry's own promotion and cumulative concurrent sibling
            landings in this same batch) -- not hand-derived from a prior
            comment's delta -- this deftest deliberately does NOT touch
            the maturity-tier aggregate count assertion above, which only
            asserts :total 436 (invariant across maturity promotions), not
            a hand-derived spec/implemented split -- that assertion is
            left exactly as found rather than fought over, per the
            race-safe convention for this hot-contention batch."
    (is (= :implemented (occupation/maturity "9216")))
    (is (= "https://github.com/cloud-itonami/cloud-itonami-isco-9216"
